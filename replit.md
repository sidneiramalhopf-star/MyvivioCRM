...
- **Workflow Builder**: Professional drag-and-drop visual workflow builder inspired by Zapier/Make/n8n with ultra-compact layout (40px header, 40px nav bar, maximized canvas). Features: START/END nodes (green, protected, auto-created), SVG-based straight-line connections, snap-to-port (30px threshold), port highlighting with glow effects, connection validation (prevents cycles using DFS, multiple inputs, self-connections), hover effects on edges, click-to-delete connections, mini toolbar on nodes (Edit/Duplicate/Delete), node selection, deep-clone duplication, zoom/pan controls, localStorage persistence keyed by journeyId. **Dual Insertion System**: (1) Card + buttons on sidebar elements open nested sidebar for configuration (mode: 'card'), (2) Edge + buttons between nodes insert elements directly without opening sidebar (mode: 'edge', uses lastInsertedType). **Nested Sidebar**: Click + on sidebar cards to configure elements with 14 trigger types (Aula reservada, Ausência, App VIVIO, Conta VIVIO, Serviço realizado, Sessões, Tarefa a fazer, Tarefa concluída, Tempo, Assinatura ativa/cancelada, Meta atingida, Treino completado, Avaliação física). Includes 12 node types: 3 REGRAS (Aguardar, Condição, Sair), 7 AÇÕES (Tarefa, Enviar mensagem, E-mail, Questionário, Tipo de contato, Atualizar Perfil, Gamificação), 2 INOVAÇÕES (Aguardar Tempo/Delay, IA Assistente). **Mini Dashboard**: Top of sidebar displays real-time metrics (Taxa de Sucesso 87%, Execuções 1.2k, Usuários 453) with professional gradient (#123058→#62b1ca). **Component Renderers**: Advanced form components (renderNumberInput, renderSelect, renderCheckboxGroup, renderMultiselect, renderInfo) for dynamic nested sidebar configuration. **Recent Updates (Nov 15, 2025)**: Added new "Inovações VIVIO" category with advanced nodes (Delay for temporal delays, IA Assistente for AI-driven predictions, Atualizar Perfil for profile updates, Gamificação for points/badges/challenges), implemented mini dashboard with gradient background at sidebar top, expanded ELEMENT_DEFINITIONS with new node types and appropriate icons/colors, created reusable form component renderers, incremented version to v5 for cache refresh.
